...
@Component({
  ...
  `,
  styles: [`
    .debug-banner { padding: 8px 12px; background: #fff3e0; color: #bf360c; border-radius: 6px; margin-bottom: 8px; font-weight: 600; }
...
export class SkChatComponent implements OnInit {
  ...
  ngOnInit(): void {
    // quick mount marker and backend health check to help debug missing DOM issue
    this.debugMessage = 'SK Chat mounted';
    // No health check on mount as it might clutter the UI, but we'll show we're mounted.
  }
